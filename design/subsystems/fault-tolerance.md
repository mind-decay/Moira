...
**Markov retry optimization:** Retry count may be reduced from the hard maximum by the Markov retry optimizer (`retry.sh`) when historical data shows low success probability. The optimizer uses exponential moving average of past retry outcomes per (error_type, agent_type) pair to estimate success probability. Hard limits remain as upper bounds — the optimizer can recommend fewer retries, never more. Report includes: "Retry recommended (estimated N% success probability based on M historical observations)" or "Escalating to user (estimated N% success probability — retry unlikely to help)."
