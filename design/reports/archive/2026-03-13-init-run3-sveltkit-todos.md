...
Чтение `.moira/config.yaml` — файл не существует. Fresh init, продолжение.
...
moira_bootstrap_generate_config "..." ".moira/state/init/tech-scan.md"
```
...
```bash
source ~/.claude/moira/lib/bootstrap.sh
moira_bootstrap_generate_project_rules "..." ".moira/state/init"
```
...
**`.moira/config.yaml`** — обновлён существующий scaffold-шаблон:
...
```bash
source ~/.claude/moira/lib/bootstrap.sh
moira_bootstrap_populate_knowledge "..." ".moira/state/init"
```
