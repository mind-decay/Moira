...
#### `.moira/core/rules/quality/` — ПУСТАЯ директория
...
#### `.moira/core/rules/roles/` — ПУСТАЯ директория

Аналогично quality/. Role definitions (hermes.yaml, hephaestus.yaml и т.д.) живут глобально. Локальная директория пуста.

**Влияние на runtime:** Зависит от того, где оркестратор ищет roles — в `{project}/.moira/core/rules/roles/` или `~/.claude/moira/core/rules/roles/`. Если в проектной — сломается.
...
Gitignore упоминает `.moira/state/current.yaml`, но файл не создан. Видимо, создаётся при первом `/moira:task`.

#### Отсутствующий `state/bypass-log.yaml`
Gitignore упоминает `.moira/state/bypass-log.yaml`, но файл не создан. Создаётся при первом `/moira:bypass`.
