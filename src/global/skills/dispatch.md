...
Instruction files are written by multiple sources:
- **Pre-planning agents (Apollo, Hermes, Athena):** Generated by `task-submit.sh` hook (Apollo at task init) and `pipeline-dispatch.sh` hook (Hermes/Athena at dispatch time) via `preflight-assemble.sh` (D-200)
- **Post-planning agents (Hephaestus, Themis, Aletheia):** Written by Daedalus (planner) during the planning step
- All instruction files contain L1-L4 merged rules, authorized knowledge, quality checklist, task context, and output path. When graph data is available, instruction files may also contain a `## Project Graph` section.

### Simplified Assembly (Fallback)

Used for:
- Metis (architect), Daedalus (planner) — need upstream artifacts not available at pre-assembly time
- Quick pipeline: all agents — no Planner step exists, no pre-assembly hooks for quick-specific agents
- Any agent when instruction file is missing (safety fallback — D-200 graceful degradation)
- Analytical pipeline: all agents (D-126)
...
| Standard | Apollo, Hermes, Athena (D-200), Hephaestus, Themis, Aletheia | Metis, Daedalus |
| Full | Apollo, Hermes, Athena (D-200), Hephaestus, Themis, Aletheia | Metis, Daedalus |
| Decomposition | Apollo, Hermes, Athena (D-200), Hephaestus, Themis, Aletheia | Metis, Daedalus |
